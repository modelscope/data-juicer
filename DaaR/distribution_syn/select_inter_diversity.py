...
def main(args):
    ...
    # Select purest data points for each category
    selected_indices = {}
    for label, (start, end) in category_ranges.items():
        current_embeddings = embeddings_tensor[start:end]
        similarities = []
        for idx, embedding in enumerate(current_embeddings):
            similarity_sum = sum(
                F.cosine_similarity(embedding.unsqueeze(0), center.unsqueeze(0)).item()
                for other_label, center in centers.items() if other_label != label
            )
            similarities.append((similarity_sum, idx))
        similarities.sort()
        selected_indices[label] = [idx for _, idx in similarities[args.lower:args.upper]]
