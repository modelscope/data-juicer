...
def fusion(model, tokenizer, de_duplicator, category):
    ...
    messages = [
        {"role": "system", "content": config_cn.system_prompt},
        {"role": "user", "content": config_cn.user_prompt_background.format(category=category)}
    ]
    ...
    messages.extend([
        ...
        {"role": "user", "content": config_cn.user_prompt_subquestion}
    ])
    text = tokenizer.apply_chat_template(
        messages,
        tokenize=False,
    ...
    messages.extend([
        ...
        {"role": "user", "content": config_cn.user_prompt_multihop}
    ])
    text = tokenizer.apply_chat_template(
        messages,
        tokenize=False,
    ...
    messages.extend([
        ...
        {"role": "user", "content": config_cn.extract_prompt_qa}
    ])
    text = tokenizer.apply_chat_template(
        messages,
        tokenize=False,
    ...
    for category in CATEGORIES:
        ...
        for i in tqdm(range(1, per_category_num + 1), desc="Running Inference"):
            try:
                ...
                with open(os.path.join(output_dir, f"thinking-cn.json"), "w", encoding="utf-8") as f:
