...
class VideoDurationFilter(Filter):
    ...
    def compute_stats(self, sample, context=False):
        ...
        if self.video_key not in sample or not sample[self.video_key]:
            sample[Fields.stats][StatsKeys.video_duration] = np.array(
                [], dtype=np.float64)
            return sample
        ...
        try: 
            ...
            sample[Fields.stats][StatsKeys.video_duration] = [0]*len(loaded_video_keys)
            return sample
        ...
        for video_key, video in videos.items():
            ...
            try:
                stream = video.streams.video[0]
                video_durations[video_key] = math.ceil(stream.duration *
                                                stream.time_base)
            except:
                video_durations[video_key] = 0
            if not context:
                video.close()
