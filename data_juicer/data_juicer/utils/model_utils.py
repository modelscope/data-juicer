...
def prepare_huggingface_model(pretrained_model_name_or_path,
    ...
    if return_model:
        ...
        try:
            ...
            if pretrained_model_name_or_path.split('/')[-1] == 'wav2vec2-large-robust-24-ft-age-gender':
                from data_juicer.my_pretrained_method.audio_code.wav2vec_age_gender import AgeGenderModel
                model = AgeGenderModel.from_pretrained(pretrained_model_name_or_path)
            else:
                raise ValueError('New model.')
...
import transformers
def prepare_llama3_model(pretrained_model_name_or_path):
...
def prepare_LlavaOnevision_model(pretrained_model_name_or_path):
    # Load the model in half-precision
    model = LlavaOnevisionForConditionalGeneration.from_pretrained(pretrained_model_name_or_path, torch_dtype=torch.float16, device_map="cpu")
...
def prepare_CogVLM2_model(pretrained_model_name_or_path):
...
def prepare_Qwen2_5_model(pretrained_model_name_or_path):
...
MODEL_FUNCTION_MAPPING = {
    ...
    'llama3': prepare_llama3_model,
    'Qwen2.5': prepare_Qwen2_5_model,
    'VideoLLaMA2': prepare_VideoLLaMA2_model,
    'CogVLM2': prepare_CogVLM2_model,
    'LlavaOnevision': prepare_LlavaOnevision_model
}
...
def move_to_cuda(model, rank):
    # add
    if str(type(model)) == "<class 'model.faceDetector.s3fd.S3FD'>":
        model = model.net
...
def get_model(model_key=None, rank=None):
    ...
    if use_cuda():
        ...
        if rank == 1:
            print(1)
            if 'prepare_light_asd_model' in str(model_key):
                print(1)
