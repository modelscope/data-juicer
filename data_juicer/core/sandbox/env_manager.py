...
class CondaEnv(Env):
    ...
    def exists(self):
        ...
        """
        try:
            res = subprocess.run([self.env_manager, 'env', 'list', '--json'],
    ...
    def install_py_deps(self, deps: Union[str, List[str]]):
        ...
        if isinstance(deps, str):
            ...
            raise TypeError(
                f'deps must be a string or a list, got {type(deps)}')
...
class VirtualEnv(Env):
    ...
    def exists(self):
        ...
        Check if an environment exists.
        """
        return os.path.exists(self.env_name) and os.path.isdir(
            self.env_name) and os.path.exists(
                os.path.join(self.env_name, 'bin', 'activate'))
