...
from data_juicer.core.sandbox.data_pool_manipulators import (
    BaseDataPoolManipulator, check_io_paths)
...
class COCOCaptionMetaGeneration(BaseDataPoolManipulator):
    ...
    def run(self):
        ...
        default_meta = {
            'root': 'data/coco/',
            'annotation':
            'data/coco/annotations/coco_karpathy_train_567k.jsonl',
            'data_augment': False,
            'repeat_time': 1,
            'length': 566747
        }
        ...
        for src_path in tqdm(existing_input_paths,
            ...
            ds = []
            default_meta['annotation'] = os.path.relpath(
                os.path.abspath(src_path), data_root_path)
            with jl.open(src_path, 'r') as reader:
                for s in reader:
                    ds.append(s)
