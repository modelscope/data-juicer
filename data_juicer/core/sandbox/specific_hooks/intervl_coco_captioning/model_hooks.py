...
class InternVLCOCOCaptionTrainExecutor(BaseModelExecutor):
    ...
    def __init__(self, model_config: dict, watcher=None):
        ...
        self.env = ENV_ROUTER[internvl_env_manager](
            env_name=internvl_env,
            env_manager=internvl_env_manager,
            **internvl_env_params)
        self.env.create()
        if self.internvl_home is None:
            raise ValueError(
                'Please specify the `internvl_home` in the config.')
        ...
        batch_size_per_device = self.model_config.get('batch_size_per_device',
                                                      4)
        work_dir = self.model_config.get('work_dir', None)
        model_name_or_path = self.model_config.get('model_name_or_path', None)
        conv_style_or_scale = self.model_config.get('conv_style_or_scale',
                                                    None)
        ...
        if conv_style_or_scale in self.SCALE_MAPPING:
            ...
            raise ValueError(
                f'conv_style_or_scale [{conv_style_or_scale}] is not supported.'
            )
