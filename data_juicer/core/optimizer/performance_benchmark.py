...
class PerformanceBenchmark:
    ...
    def run_individual_filters_benchmark(self, filters: List[Filter], test_data: Dict[str, Any]) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(total_time=total_time,
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        logger.info(f"  Individual Execution: {results['individual']['mean_total_time']:.3f}s "
                    f"± {results['individual']['std_total_time']:.3f}s")
        logger.info(f"  Fused Execution: {results['fused']['mean_total_time']:.3f}s "
...
def run_simple_demo():
    ...
    logger.info(f'DEBUG: type(samples)={type(samples)}, '
                f'keys={list(samples.keys()) if isinstance(samples, dict) else "N/A"}')
    ...
    filter_speedup = individual_stats['filter_time'] / fused_stats['filter_time'] if fused_stats[
        'filter_time'] > 0 else float('inf')

    logger.info('\n🎯 IMPROVEMENTS:')
    logger.info(f'  Total Speedup: {total_speedup:.2f}x')
    logger.info(f'  Time Saved: {time_saved:.3f}s '
                f'({time_saved/individual_stats["total_time"]*100:.1f}%)')
    ...
    logger.info(f'Created {len(complex_filters)} complex filters: '
                f'{[f._name for f in complex_filters]}')
...
def collect_filtering_stats(filters: List[Filter], test_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for i, filter_op in enumerate(filters):
        ...
        logger.info(f'    Passed: {passed_samples:,}/{total_samples:,} '
                    f'({pass_rate:.1f}%)')
    ...
    logger.info(f'    Fused - Passed: {fused_passed:,}/{total_samples:,} '
                f'({fused_pass_rate:.1f}%)')
...
def print_filtering_comparison(stats: Dict[str, Any]):
    ...
    logger.info(f'  Fused Execution:        {fused_passed:8,} passed '
    ...
    efficiency_ratio = fused_filtered / total_individual_filtered if total_individual_filtered > 0 else 1.0  # noqa: E501
...
def analyze_fusion_decisions():
    ...
    filter_combinations = [
        # Simple filters only
        {
            'name': 'Simple Only',
            'filters': create_simple_filters(),
            'expected': 'skip_fusion'
        },
        # Complex filters only
        {
            'name': 'Complex Only',
            'filters': create_complex_filters(),
            'expected': 'use_fusion'
        },
        # Mixed filters
        {
        ...
        {
            'name': 'Single Filter',
            'filters': [create_simple_filters()[0]],
            'expected': 'skip_fusion'
        },
        # Two simple filters
        {
            'name': 'Two Simple',
            'filters': create_simple_filters()[:2],
            'expected': 'skip_fusion'
        }
    ]
...
def main():
    ...
    if args.mode == 'demo':
        ...
        logger.info(f'Running in COMPREHENSIVE mode with {args.samples:,} samples, '
                    f'{args.runs} runs')
