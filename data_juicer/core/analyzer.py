...
class Analyzer:
    ...
    def get_splitting_recommendations(self):
        """
        Analyze dataset characteristics and provide splitting recommendations
        for robust data processing.

        Returns:
            dict: Splitting recommendations with reasons
        """
        ...
        # Check dataset size
        total_samples = len(self.dataset_builder.load_dataset())
        if total_samples > 1000000:  # 1M samples
            recommendations["should_split"] = True
            recommendations["split_reasons"].append("Large dataset size")
            recommendations["optimal_chunk_size"] = 100000

        # Check for skewed distributions
        if "text_length" in self.overall_result.columns:
            text_stats = self.overall_result["text_length"]
            if text_stats.get("std", 0) / text_stats.get("mean", 1) > 2.0:
                recommendations["should_split"] = True
                recommendations["split_reasons"].append("High text length variance")
                recommendations["split_strategy"] = "stratified_by_length"
        ...
        multimodal_count = sum(
            ...
            if indicator in self.overall_result.columns and self.overall_result[indicator].get("mean", 0) > 0.1
        )

        if multimodal_count > 1:
            recommendations["should_split"] = True
            recommendations["split_reasons"].append("Mixed content modalities")
            recommendations["split_strategy"] = "split_by_modality"
