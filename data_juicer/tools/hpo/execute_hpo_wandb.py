...
from data_juicer.tools.hpo.objects import get_hpo_objective


def main():
    """Run a W&B sweep using the search space supplied by ``--hpo_config``."""
    ...
    if not sweep_cfg_file_path:
        raise ValueError(
            "Not found --hpo_config, you should specify your " "hpo cfg file path following `--hpo_config`"
        )
