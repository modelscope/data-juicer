...
class Difference_Caption_Generator_Mapper(Mapper):
    ...
    def __init__(self,
        ...
        self.text_pair_similarity_filter_args = self._prepare_op_args("text_pair_similarity_filter", text_pair_similarity_filter_args)
    ...
    def process_single(self, samples, rank=None):
        ...
        crop_image1_filtered = crop_image1_samples.map(self.fused_ops[1].compute_stats,
                              num_proc=self.image_text_matching_filter_args["num_proc"],
                              with_rank=True)
        crop_image1_filtered = crop_image1_filtered.filter(self.fused_ops[1].process, num_proc=self.image_text_matching_filter_args["num_proc"])
        ...
        crop_image2_filtered = crop_image2_samples.map(self.fused_ops[1].compute_stats,
                              num_proc=self.image_text_matching_filter_args["num_proc"],
                              with_rank=True)
        crop_image2_filtered = crop_image2_filtered.filter(self.fused_ops[1].process, num_proc=self.image_text_matching_filter_args["num_proc"])
        ...
        if len(filtered_caption_pairs) == 0:
            return {Fields.meta: {"region_caption1": [], "region_caption2": [], MetaKeys.bbox_tag: np.empty((0, 0, 4), dtype=np.float32), "bbox_difference_captions": []}}
        ...
        if Fields.stats not in filtered_caption_pairs.features:
            filtered_caption_pairs = filtered_caption_pairs.add_column(name=Fields.stats,
                                         column=[{}] * filtered_caption_pairs.num_rows)

        filtered_caption_pairs = filtered_caption_pairs.map(self.fused_ops[2].compute_stats,
                              num_proc=self.text_pair_similarity_filter_args["num_proc"],
                              with_rank=True)
        filtered_caption_pairs = filtered_caption_pairs.filter(self.fused_ops[2].process, num_proc=self.text_pair_similarity_filter_args["num_proc"])
        ...
        for temp_bbox_id in seen:
            if crop_bbox_id_to_caption1[temp_bbox_id] in effective_bboxes_caption1 and crop_bbox_id_to_caption2[temp_bbox_id] in effective_bboxes_caption2:
                temp_bbox_json = {}
        ...
        for temp_bbox_id, temp_bbox_json in enumerate(effective_bboxes):
            ...
            cv2.rectangle(temp_image_array1, (int(extend_x1), int(extend_y1)), (int(extend_x2), int(extend_y2)), (0, 0, 255), 3)
            cv2.rectangle(temp_image_array2, (int(extend_x1), int(extend_y1)), (int(extend_x2), int(extend_y2)), (0, 0, 255), 3)            
            ...
            valid_img_name = samples["image_path1"].split("/")[-1].split(".")[-2]
            temp_image_path = os.path.join(DATA_JUICER_ASSETS_CACHE, f"red_box_images_{valid_img_name}_{random_num}_<{str(temp_bbox_id)}>.jpg")
            cv2.imwrite(temp_image_path, concat_image)
            cache_image_list.append(temp_image_path)

            text_key = "Analyse the left image and the right image (separated by the black vertical bar). The detail within the red bounding box in the left image is: " + \
                        temp_bbox_json["caption1"] + ", the detail within the red bounding box in the right image is: " + temp_bbox_json["caption2"] + \
                        ". What is their difference? Answer with a few concise sentences."
