...
import data_juicer
from data_juicer.ops.base_op import OP, OPERATORS, Filter, Mapper, TAGGING_OPS, UNFORKABLE
...
class Difference_Area_Generator_Mapper(Mapper):
    ...
    def __init__(self,
        ...
        self.image_pair_similarity_filter_args = self._prepare_op_args("image_pair_similarity_filter", image_pair_similarity_filter_args)
        self.image_segment_mapper_args = self._prepare_op_args("image_segment_mapper", image_segment_mapper_args)
        self.image_text_matching_filter_args = self._prepare_op_args("image_text_matching_filter", image_text_matching_filter_args)
    ...
    def process_single(self, samples, rank=None):
        ...
        # fused_ops 1.image_pair_similarity_filter 2.image_segment_mapper 3.image_text_matching_filter
        # keys of sample: "caption1", "caption2", "image_path1", "image_path2"

        # Step1: filter out image pairs with large differences between the two images.
        temp_sample = {}
        ...
        if not new_samples_s1:
            return {Fields.meta: {MetaKeys.bbox_tag: np.empty((0, 0, 4), dtype=np.float32)}}



        # Step2: compare the differences between the two captions and identify the "valid object".
        valid_object1, valid_object2 = compare_text_index(samples["caption1"], samples["caption2"])
        


        # Step3: segment the regions in two images that may contain valid objects.
        temp_sample = {}
        temp_sample["images"] = [samples["image_path1"], samples["image_path2"]]
        temp_sample = data_juicer.core.NestedDataset.from_list([temp_sample])
        ...
        # Step4: crop sub-images based on the bounding boxes for subsequent image-text matching processes.
        crop_image1_samples = []
        crop_image2_samples = []
        ...
        for temp_bbox_id, temp_bbox in enumerate(image1_bboxes):
            crop_img = image_array1[int(temp_bbox[1]-temp_bbox[3]/2):int(temp_bbox[1]+temp_bbox[3]/2), int(temp_bbox[0]-temp_bbox[2]/2):int(temp_bbox[0]+temp_bbox[2]/2), :]
            valid_img_name = samples["image_path1"].split("/")[-1].split(".")[-2]
            temp_image_path = os.path.join(DATA_JUICER_ASSETS_CACHE, f"cropped_images_{valid_img_name}_{random_num}_1_<{str(temp_bbox_id)}>.jpg")
            ...
            for temp_valid_object1 in valid_object1:
                crop_image1_samples.append({"text": f'{SpecialTokens.image}' + temp_valid_object1 + f'{SpecialTokens.eoc} ', "images": [temp_image_path]})

            crop_img_another = image_array2[int(temp_bbox[1]-temp_bbox[3]/2):int(temp_bbox[1]+temp_bbox[3]/2), int(temp_bbox[0]-temp_bbox[2]/2):int(temp_bbox[0]+temp_bbox[2]/2), :]
            valid_img_name = samples["image_path2"].split("/")[-1].split(".")[-2]
            temp_image_path = os.path.join(DATA_JUICER_ASSETS_CACHE, f"cropped_images_{valid_img_name}_{random_num}_2_<{str(temp_bbox_id)}>.jpg")
            ...
            for temp_valid_object2 in valid_object2:
                crop_image2_samples.append({"text": f'{SpecialTokens.image}' + temp_valid_object2 + f'{SpecialTokens.eoc} ', "images": [temp_image_path]})
        ...
        for temp_bbox_id, temp_bbox in enumerate(image2_bboxes):
            ...
            crop_img = image_array2[int(temp_bbox[1]-temp_bbox[3]/2):int(temp_bbox[1]+temp_bbox[3]/2), int(temp_bbox[0]-temp_bbox[2]/2):int(temp_bbox[0]+temp_bbox[2]/2), :]
            valid_img_name = samples["image_path2"].split("/")[-1].split(".")[-2]
            temp_image_path = os.path.join(DATA_JUICER_ASSETS_CACHE, f"cropped_images_{valid_img_name}_{random_num}_2_<{str(temp_crop_image_pair_id)}>.jpg")
            cv2.imwrite(temp_image_path, crop_img)
            crop_image2_path_to_bbox_dict[temp_image_path] = temp_bbox
            for temp_valid_object2 in valid_object2:
                crop_image2_samples.append({"text": f'{SpecialTokens.image}' + temp_valid_object2 + f'{SpecialTokens.eoc} ', "images": [temp_image_path]})

            crop_img_another = image_array1[int(temp_bbox[1]-temp_bbox[3]/2):int(temp_bbox[1]+temp_bbox[3]/2), int(temp_bbox[0]-temp_bbox[2]/2):int(temp_bbox[0]+temp_bbox[2]/2), :]
            valid_img_name = samples["image_path1"].split("/")[-1].split(".")[-2]
            temp_image_path = os.path.join(DATA_JUICER_ASSETS_CACHE, f"cropped_images_{valid_img_name}_{random_num}_1_<{str(temp_crop_image_pair_id)}>.jpg")
            cv2.imwrite(temp_image_path, crop_img_another)
            crop_image1_path_to_bbox_dict[temp_image_path] = temp_bbox
            for temp_valid_object1 in valid_object1:
                crop_image1_samples.append({"text": f'{SpecialTokens.image}' + temp_valid_object1 + f'{SpecialTokens.eoc} ', "images": [temp_image_path]})
        ...
        if Fields.stats not in crop_image1_samples.features:
            crop_image1_samples = crop_image1_samples.add_column(name=Fields.stats,
                                         column=[{}] * crop_image1_samples.num_rows)
        crop_image1_filtered = crop_image1_samples.map(self.fused_ops[2].compute_stats,
                              num_proc=self.image_text_matching_filter_args["num_proc"],
                              with_rank=True)
        crop_image1_filtered = crop_image1_filtered.filter(self.fused_ops[2].process, num_proc=self.image_text_matching_filter_args["num_proc"])
        ...
        if Fields.stats not in crop_image2_samples.features:
            crop_image2_samples = crop_image2_samples.add_column(name=Fields.stats,
                                         column=[{}] * crop_image2_samples.num_rows)
        crop_image2_filtered = crop_image2_samples.map(self.fused_ops[2].compute_stats,
                              num_proc=self.image_text_matching_filter_args["num_proc"],
                              with_rank=True)
        crop_image2_filtered = crop_image2_filtered.filter(self.fused_ops[2].process, num_proc=self.image_text_matching_filter_args["num_proc"])
        ...
        for temp_crop_image1_filtered in crop_image1_filtered:
            ...
            if temp_bbox_id in crop_image2_filtered_bbox_id:
                ...
                temp_filtered_sub_image_pairs["images"] = [temp_crop_image1_filtered["images"][0], temp_crop_image1_filtered["images"][0].replace(valid_image_path1, valid_image_path2).replace("_1_<", "_2_<")]
                filtered_sub_image_pairs.append(temp_filtered_sub_image_pairs)



        # Step6: determine whether there are differences in the two images corresponding to each bounding box.
        ...
        if Fields.stats not in filtered_sub_image_pairs.features:
            filtered_sub_image_pairs = filtered_sub_image_pairs.add_column(name=Fields.stats,
                                         column=[{}] * filtered_sub_image_pairs.num_rows)
        filtered_sub_image_pairs = filtered_sub_image_pairs.map(self.fused_ops[0].compute_stats,
                            num_proc=self.image_pair_similarity_filter_args["num_proc"],
                            with_rank=True)
        filtered_sub_image_pairs = filtered_sub_image_pairs.filter(self.fused_ops[0].process, num_proc=self.image_pair_similarity_filter_args["num_proc"])
