...
class ImageAspectRatioFilter(Filter):
    """Filter to keep samples with image aspect ratio within a specific range. Aspect ratio is
    defined as the width divided by the height (W / H).

    - The operator computes the aspect ratio for each image in the sample.
    - It caches the computed aspect ratios in the 'aspect_ratios' field.
    - Samples are kept if their images' aspect ratios fall within the specified
    minimum and maximum range.
    - The 'any_or_all' parameter determines the strategy: 'any' keeps samples if
    at least one image meets the criteria, while 'all' requires all images to
    meet the criteria.
    - If no images are present in a sample, the sample is not filtered out."""
    ...
    def process_batched(self, samples):
        def process_single(values):
            keep_bools = np.array([self.min_ratio <= value <= self.max_ratio for value in values])
            if len(keep_bools) <= 0:
                return True
