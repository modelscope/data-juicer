...
class VideoTaggingFromFramesFilter(Filter):
    ...
    "uniform", the number of frames to sample can be specified. The operator stores the
    extracted tags in the meta field with the key 'video_frame_tags' by default. The
    decision to keep a sample is based on whether any or all of the video frames meet the
    tag criteria, as specified by the 'any_or_all' parameter."""
