...
class InContextInfluenceFilter(LLMPerplexityFilter):
    ...
    This operator computes the in-context influence of each sample on a provided validation
    set. The influence is calculated as the ratio of perplexities with and without the
    sample as context. If `valid_as_demo` is True, the score is computed as L(A|Q) /
    L(A|task_desc, Q_v, A_v, Q). Otherwise, it is L(A_v|Q) / L(A_v|task_desc, Q, A, Q_v).
    The operator keeps samples whose in-context influence score falls within a specified
