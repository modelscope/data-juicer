...
def _worker_process(config_path: str, mode: str, result_queue: mp.Queue):
    ...
    def run_benchmark_comparison(self, recipe_path: str, dataset_path: str) -> Dict[str, Any]:
        ...
        optimizer = PipelineOptimizer(
            [FilterFusionStrategy(), MapperFusionStrategy()],
    ...
    def validate_results(self, individual_result: Dict, optimized_result: Dict) -> Dict[str, Any]:
        ...
        if individual_result.get("success") and optimized_result.get("success"):
            ...
            if validation["validation_passed"]:
                ...
                logger.warning(f"❌ Validation failed: Sample count mismatch "
        ...
        with open(report_path, 'w') as f:
            ...
            if validation['validation_passed']:
                if comparison['faster_mode'] == 'optimized':
                    f.write("✅ **Use Optimized Pipeline**: The optimized pipeline is faster and produces correct results.\n")
                elif comparison['faster_mode'] == 'individual':
                    f.write("⚠️ **Consider Individual Pipeline**: The individual pipeline is faster, but optimization may still be beneficial for larger datasets.\n")
                else:
                    f.write("ℹ️ **Both Modes Similar**: Performance is similar between individual and optimized modes.\n")
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Pipeline Performance Test: Compare individual vs optimized execution"
    )
    parser.add_argument(
        "--recipe-path", 
        type=str, 
