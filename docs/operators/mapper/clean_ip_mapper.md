...
#### ✨ explanation 解释
This example shows how the operator removes IPv4 addresses from the text. The IP addresses are replaced with an empty string, which effectively deletes them. For instance, '234.128.124.123' is removed from 'test of ip 234.128.124.123', leaving only 'test of ip '.
这个例子展示了算子如何从文本中移除IPv4地址。IP地址被替换为空字符串，这实际上就是删除了它们。例如，'test of ip 234.128.124.123'中的'234.128.124.123'被移除后，只剩下'test of ip '。
...
#### ✨ explanation 解释
In this example, the operator replaces IPv4 addresses in the text with a custom string '<IP>'. This means that instead of removing the IP addresses, they are marked. For example, '234.128.124.123' is replaced by '<IP>' in 'test of ip 234.128.124.123', resulting in 'test of ip <IP>'.
在这个例子中，算子用自定义字符串'<IP>'替换文本中的IPv4地址。这意味着IP地址不是被删除而是被标记。例如，'test of ip 234.128.124.123'中的'234.128.124.123'被替换为'<IP>'，结果变为'test of ip <IP>'。
