...
此运算符根据样本的总文本长度过滤样本。它保留文本长度在指定的最小和最大长度之间的样本。文本长度计算为样本文本中的字符数。如果 'text_len' 键已经存在于样本的统计信息中，则将重用它; 否则，将计算它。为了提高效率，操作员分批处理样品。
...
|--------|------|--------|------|
| `min_len` | <class 'int'> | `10` | The min text length in the filtering. samples |
| `max_len` | <class 'int'> | `9223372036854775807` | The max text length in the filtering. samples |
| `args` |  | `''` | extra args |
| `kwargs` |  | `''` | extra args |
