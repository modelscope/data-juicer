...
SpecialCharactersFilter 算子根据文本中特殊字符的比例筛选样本，保留特殊字符比例在0.0到0.25之间的样本。第一个、第二个和第五个样本被保留，因为它们的特殊字符比例在这个范围内。第三个样本由于没有特殊字符导致比例低于最小阈值而被移除。第四个和第六个样本的特殊字符比例过高，超过了最大阈值，因此也被移除。


## 🔗 related links 相关链接
- [source code 源代码](../../../data_juicer/ops/filter/special_characters_filter.py)
- [unit test 单元测试](../../../tests/ops/filter/test_special_characters_filter.py)
