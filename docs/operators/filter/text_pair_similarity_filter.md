...
过滤以将具有相似性的文本对保持在特定范围内。

该运算符使用拥抱面部剪辑模型计算一对中的两个文本之间的相似性。它保留相似性得分落在指定的最小和最大阈值内的样本。关键度量 “text_pair_similarity” 被计算为文本嵌入之间的余弦相似度。该运算符支持两种保持样本的策略: 'any' (如果任何对满足条件，则保持) 和 'all' (仅当所有对满足条件时保持)。如果未提供第二个文本键，则操作员将引发错误。相似性得分缓存在样本统计信息中的 “text_pair_similarity” 字段下。
