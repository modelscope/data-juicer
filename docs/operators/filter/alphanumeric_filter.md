...
过滤器，以保持具有特定范围内的字母/数字比率的样本。

此运算符根据字母数字字符或标记的比率筛选样本。它会保留字母数字字符 (或标记) 与字符 (或标记) 总数之比在指定范围内的样本。根据 'tokenization' 参数，基于字符或基于令牌计算该比率。如果 'tokenization' 是真的，它使用一个拥抱的脸标记器来计算标记。如果启用了标记化，则用于过滤的关键度量是 “alpha_token_ratio”，否则为 “alnum_ratio”。运算符将这些度量缓存在每个样本的stats字段中。
...
算子基于字母数字字符与总字符的比例筛选样本，仅保留比例在0.2到0.9之间的样本。第四个样本被移除是因为其字母数字字符比例不在这个范围内。
...
当启用分词时，算子使用Hugging Face分词器来计算标记，并根据字母数字标记比率筛选样本。只有两个样本满足字母数字标记比率大于1.5的标准，因此其他样本被移除。


## 🔗 related links 相关链接
- [source code 源代码](../../../data_juicer/ops/filter/alphanumeric_filter.py)
- [unit test 单元测试](../../../tests/ops/filter/test_alphanumeric_filter.py)
