# Some codes are adapted from https://pypi.org/project/transformers
...
_CONFIG_FOR_DOC = "MegatronLlamaConfig"

LLAMA_PRETRAINED_CONFIG_ARCHIVE_MAP = {}


class MegatronLlamaConfig(PretrainedConfig):
    model_type = "megatron-llama"
...
class LlamaRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        super().__init__()
        inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2).float().to(device) / dim))  # noqa: E225
...
class LlamaPreTrainedModel(PreTrainedModel):
    config_class = MegatronLlamaConfig
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: MegatronLlamaConfig):
        ...
        #  embedding
        self.embed_position = nn.Embedding(config.max_sequence_length, config.hidden_size)
    ...
    def forward(
        ...
        position_embeds = self.embed_position(position_ids)
...
class MegatronLlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        >>> model = MegatronLlamaForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
