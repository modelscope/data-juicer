...
def find_lazy_loaders(file_path: Path) -> List[Tuple[str, str]]:
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                if isinstance(node.func,
                              ast.Name) and node.func.id == 'LazyLoader':
                    ...
                    if node.args and isinstance(node.args[0], ast.Constant):
                        ...
                        if len(node.args) > 1 and isinstance(
                            ...
                            for kw in node.keywords:
                                if kw.arg == 'package_name' and isinstance(
        ...
        logger.warning(
            f'Failed to parse LazyLoader instances from {file_path}: {e}')
    return lazy_loaders
...
def get_operator_imports(op_name: str) -> Set[str]:
    ...
    for op_dir in op_dirs:
        op_paths.extend([
            Path(__file__).parent.parent / 'data_juicer' / 'ops' / op_dir /
            f'{op_name}.py',
            Path(__file__).parent.parent / 'data_juicer' / 'ops' / op_dir /
            op_name / '__init__.py',
        ])
