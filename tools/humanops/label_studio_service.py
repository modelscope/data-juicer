...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=
        'Directory to store Label Studio data (only used with --use-host-mount)'
    )
    parser.add_argument(
        '--username',
    ...
    parser.add_argument(
        '--remove-volumes',
        action='store_true',
        help='Remove Docker volumes when stopping the container')
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=
        'Use host directory mounting instead of Docker volumes (not recommended on macOS)'
    )
    parser.add_argument(
        '--use-pip',
        action='store_true',
        help=
        'Use local pip installation instead of Docker (uses a virtual environment)'
    )
...
def start_label_studio_container(port,
    ...
    if use_host_mount:
        ...
        required_dirs = [
            'media', 'data', 'upload', 'export', 'tmp', 'test_data',
            'upload/avatars'
        ]
        ...
        touch_files = [
            os.path.join(data_dir, 'label_studio.sqlite3'),
            os.path.join(data_dir, 'logs.txt')
        ]
        ...
        if os.name != 'nt':  # Not Windows
            try:
                ...
                logger.warning(
                    f'Failed to set permissions for data directory: {e}')
                logger.warning(
                    'The container may not be able to write to the data directory'
                )
    ...
    if use_host_mount:
        logger.info(
            'Using host directory mount approach (not recommended on macOS)')
        cmd.extend([
        ...
        check_result = subprocess.run(check_volume_cmd,
                                      capture_output=True,
                                      text=True,
                                      check=False)
        ...
        if volume_name not in check_result.stdout:
            ...
            create_result = subprocess.run(create_volume_cmd,
                                           capture_output=True,
                                           text=True,
                                           check=False)
            ...
            if create_result.returncode != 0:
                logger.error(
                    f'Failed to create Docker volume: {create_result.stderr}')
    ...
    if result.returncode != 0:
        ...
        if 'port is already allocated' in result.stderr:
            ...
            import re
            error_match = re.search(r'Error response from daemon: (.*)',
                                    result.stderr)
            if error_match:
        ...
        if check_container_exists(container_name):
            logger.info(
                f'Cleaning up partially created container: {container_name}')
            stop_container(container_name)
        ...
        if not check_container_running(container_name):
            ...
            if exit_info:
                if 'exit_code' in exit_info:
                    logger.info(
                        f"Container exit code: {exit_info['exit_code']}")
            ...
            if log_result.returncode == 0:
                ...
                if not log_result.stdout.strip(
                ) and not log_result.stderr.strip():
                    logger.info(
                        'No logs found. Trying to get container information...'
                    )
                    ...
                    if inspect_result.returncode == 0:
                        try:
                            ...
                            logger.info(
                                f'Failed to parse container inspect data: {e}')
            else:
            ...
            if inspect_result.returncode == 0:
                try:
                    ...
                    if container_info and len(container_info) > 0:
                        state = container_info[0].get('State', {})
                        logger.info(
                            f'Container state: {json.dumps(state, indent=2)}')
                except Exception as e:
    ...
    if check_container_running(container_name):
        ...
        logger.info('Or stop the container with: docker stop ' +
                    container_name)
    else:
        logger.info(
            'Container is not running. Check the logs above for errors.')
...
def main():
    ...
    if args.use_pip:
        ...
        if not use_docker:
            logger.warning(
                'Docker is not available. Falling back to pip installation.')
...
def start_label_studio_with_docker(args, api_token):
    ...
    for image in images_to_try:
        ...
        if server_url:
            logger.info(
                f'Successfully started Label Studio using image: {image}')
