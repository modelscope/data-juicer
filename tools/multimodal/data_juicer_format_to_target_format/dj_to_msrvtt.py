...
def main(
    ...
    original_msr_vtt_ds_path: str = None,
):
    """
    ...
    :param convert_to_relative_paths: whether convert the video paths in this
        ...
        extra argument original_msr_vtt_ds_path is required. When the
        processed and converted dataset will be used in another machine, it's
        better to set this argument to True. Default: False.
    :param original_msr_vtt_ds_path: path to the original unprocessed
        MSR-VTT dataset, which is used to help to recover the relative video
    ...
    if not target_msr_vtt_ds_path.endswith('.jsonl'):
        raise ValueError(
            'Only support "jsonl" target dataset file for MSR-VTT now.')
    if os.path.dirname(target_msr_vtt_ds_path) \
            and not os.path.exists(os.path.dirname(target_msr_vtt_ds_path)):
        logger.info(
            f'Create directory [{os.path.dirname(target_msr_vtt_ds_path)}] '
    ...
    # original_msr_vtt_ds_path is provided as well.
    if convert_to_relative_paths:
        if not original_msr_vtt_ds_path:
            raise ValueError('When convert_to_relative_paths is set to True, '
                             'the original_msr_vtt_ds_path must be provided '
                             'for recovering the relative paths. Please '
                             'check and retry.')
        original_msr_vtt_ds_path = os.path.abspath(original_msr_vtt_ds_path)
        # if provided original_msr_vtt_ds_path is the dataset file path, only
        # keep the directory path.
        if os.path.isfile(original_msr_vtt_ds_path):
            original_msr_vtt_ds_path = os.path.dirname(
                original_msr_vtt_ds_path)
    ...
    with jl.open(dj_ds_path) as reader:
        with jl.open(target_msr_vtt_ds_path, mode='w') as writer:
            for line_num, s in enumerate(tqdm(reader)):
                ...
                # add video
                if convert_to_relative_paths:
                    if video.startswith(original_msr_vtt_ds_path):
                        video = os.path.relpath(video,
                                                original_msr_vtt_ds_path)
                    else:
                        raise ValueError(
                            f'The original_msr_vtt_ds_path '
                            f'[{original_msr_vtt_ds_path}] is not the '
                            f'directory that contains the video '
                            f'[{video}] in the sample of line number '
                            f'[{line_num}]. Please check if the correct '
                            f'original_msr_vtt_ds_path is provided or '
