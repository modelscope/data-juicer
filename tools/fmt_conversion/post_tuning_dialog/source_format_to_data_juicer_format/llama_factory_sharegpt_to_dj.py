...
def llama_factory_sharegpt_to_dj(
        ...
        if conv[from_key] == system_role:
            if system_prompt != '':
                raise NotImplementedError(
                    'DO NOT support more than 1 system prompts in the '
            ...
            if instruction != '':
                raise NotImplementedError(
                    'DO NOT support more than 1 instructions in the '
                    'conversation for now.')
    ...
    if conv_num == 0:
        ...
        history = [[
            conversations[i][value_key], conversations[i + 1][value_key]
        ] for i in range(0, conv_num - 2, 2)]
    else:
