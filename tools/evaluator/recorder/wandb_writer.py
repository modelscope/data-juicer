...
def get_args():
    parser = argparse.ArgumentParser(
        description="write evaluation result into wandb",
        allow_abbrev=False
    )
...
DEFAULT_HELM_METRICS = [
    "mmlu.EM", "raft.EM", "imdb.EM", "truthful_qa.EM", "summarization_cnndm.ROUGE-2", "summarization_xsum.ROUGE-2", "boolq.EM", "msmarco_trec.NDCG@10", "msmarco_regular.RR@10",
    "narrative_qa.F1", "natural_qa_closedbook.F1", "natural_qa_openbook_longans.F1", "civil_comments.EM", "hellaswag.EM", "openbookqa.EM"
]
...
class HelmWriter(Writer):
    ...
    def __init__(self,
        ...
        if self.leaderboard:
            self.leaderboard_metrics = self.conf['leaderboard_metrics'] if 'leaderboard_metrics' in self.conf else DEFAULT_HELM_METRICS
    ...
    def make_leaderboard(self):
        ...
        for run in runs:
            ...
            history = run.scan_history(
                keys=['_step'] + self.leaderboard_metrics, page_size=2000, min_step=0)
    ...
    def parse_scenarios(self):
        ...
        for scenario in self.scenarios:
            try:
                result = self.parse_scenario(
                    scenario['name'], scenario['metrics'], self.model)
        ...
        with open(os.path.join(self.helm_root, 'runs', self.suite_name, 'groups', f'{scenario_name}.json')) as f:
            ...
            for subject in subjects:
                ...
                for row in subject['rows']:
                    ...
                    evaluate_result[iteration][scenario_name][subject['title'].split(',')[
                        0]] = dict()
                    for metric, i in record_column_idx.items():
                        evaluate_result[iteration][scenario_name][subject['title'].split(',')[
                            0]][metric] = row[i]['value']
